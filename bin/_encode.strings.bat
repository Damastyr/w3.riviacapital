...
rem ---------------------------------------------------
rem --- put custom voiceline strings into strings dir for concatenation

set CUSTOM_VOICELINES_CSV=%DIR_SPEECH%\mod%MODNAME%.speech.csv
set STRINGS_PART_CSV=%DIR_STRINGS%\%STRINGS_PART_PREFIX%custom.speech.csv

if EXIST "%CUSTOM_VOICELINES_CSV%" (
  type "%CUSTOM_VOICELINES_CSV%" > "%STRINGS_PART_CSV%"
)
...
for %%f in ("%STRINGS_PART_WILDCARD%") do (
  set FILENAME=%%~ff
    ...
    rem make sure files are separated even if they don't have a newline at fileend
    echo.; >> "%STRINGS_FILE_COMBINED%"
)
echo.
