...
rem ---------------------------------------------------
rem -- ENCODER: CONVERT HAIRWORKS

IF %ENCODE_HAIRWORKS% EQU 1 CALL "%DIR_PROJECT_BIN%\_encode.apx.bat"
IF %ERRORLEVEL% NEQ 0 GOTO SomeError
...
rem ---------------------------------------------------
rem -- ENCODER: SHADOWMESHES

IF %ENCODE_SHADOWMESHES% EQU 1 CALL "%DIR_PROJECT_BIN%\_encode.shadowmeshes.bat"
IF %ERRORLEVEL% NEQ 0 GOTO SomeError

rem ---------------------------------------------------
rem -- PREPARE COOKING - before first run of wcc

IF %WCC_COOK% EQU 1 CALL "%DIR_PROJECT_BIN%\_prepare.cooking.bat"
IF %ERRORLEVEL% NEQ 0 GOTO SomeError

rem ---------------------------------------------------
rem -- WCC_LITE: IMPORT TEXTURES

IF %WCC_IMPORT_TEXTURES% EQU 1 CALL:wccImportTextures
IF %ERRORLEVEL% NEQ 0 GOTO SomeError

rem ---------------------------------------------------
rem -- WCC_LITE: IMPORT MODELS

IF %WCC_IMPORT_MODELS% EQU 1 CALL "%DIR_PROJECT_BIN%\_wcc.import.models.bat"
IF %ERRORLEVEL% NEQ 0 GOTO SomeError
...
IF %WCC_REPACK_MOD% EQU 1 CALL "%DIR_PROJECT_BIN%\_prepare.packaging.mod.bat"
IF %ERRORLEVEL% NEQ 0 GOTO SomeError
...
SET IMPORTED_TEXTURES=0
IF EXIST "%DIR_TEXTURE_IMPORT%\texture.imports.bat" (
  call :printHeader WCC_LITE: IMPORT TEXTURES

  rem calls custom batch for importing textures
  CALL "%DIR_TEXTURE_IMPORT%\texture.imports.bat"

  IF %INTERACTIVE_BUILD% EQU 1 PAUSE
)
IF %IMPORTED_TEXTURES% == 0 (
  echo no textures imported.
  EXIT /B %ERRORLEVEL%
) else (
  rem imported textures are saved in separate dir to easily create texture cache afterwards
  rem however uncooked entities referencing new imported textures also require the *uncooked* textures
  rem -> copy all textures to uncooked folder after import

  robocopy "%DIR_UNCOOKED_TEXTURES%" "%DIR_UNCOOKED%" /s /NFL /NJH /XX
  echo  ^> done.
